...
async def question_from_user(query: types.CallbackQuery):
    ...
    await query.message.delete()
    await query.message.answer(f'Вы хотите задать вопрос Техподдержке?',
                               reply_markup=markup.as_markup(resize_keyboard=True))
...
async def send_to_support(message: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id='-1001684049768',
                           text=f'Пришел новый вопрос от пользователя!\n@{username}\n{question}', \
                           reply_markup=markup.as_markup(resize_keyboard=True))
